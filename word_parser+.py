...
class WordDocumentParser:
    ...
    def _add_metadata(self, line: str) -> Tuple[List[str], Optional[str]]:
    ...
    def convert_to_markdown(self) -> str:
        ...
        try:
            ...
            for line in processed_lines:
                processed_content, line_type = self._add_metadata(line)

                if line_type in ["table_row", "table_separator"]:
                    if not in_table and line_type == "table_row":
                        current_table = processed_content
                        in_table = True
                    elif line_type == "table_separator":
                        current_table.extend(processed_content)
                        in_table = True
                    else:
                        current_table.extend(processed_content)
                else:
                    ...
                    if line_type not in ["list_item"] and processed_content[-1] != "":
                        markdown_lines.append("")
